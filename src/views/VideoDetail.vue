          ...
          <van-collapse-item :title="videoData.name" name="1">
            <template #title>
              <div class="video-title" :class="{ 'title-wrap': titleWrap }">
                <span class="video-type">{{ videoData.category.title }}</span>  <!-- 视频类型 -->
                {{ videoData.name }}  <!-- 视频名字 -->
              </div>
              <div class="video-moreinfo">
                ...
                <span class="play-count">1865万观看</span>
                <span class="danmu">1.8万弹幕</span>
                <span class="date">{{ videoData.date }}</span> <!-- 发布时间 -->
              </div>
            </template>
